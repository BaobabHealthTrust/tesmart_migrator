...
def start
  dispensation_obs = Patient.find_by_sql("SELECT order_id, value_numeric, value_drug, person_id, obs_datetime 
    FROM #{Source_db}.obs WHERE concept_id = 2834 AND voided = 0 AND value_drug IS NOT NULL")
  count = 0
  dispensation_obs.each do |dispensation| 
    count = count + 1
    obs_id = Observation.find_by_sql("SELECT MAX(obs_id) AS our_id FROM #{Source_db}.obs WHERE  
    ...
    if ! obs_id.first.our_id.nil?
			remaining_pills = Observation.find_by_sql("SELECT value_numeric 
			  FROM #{Source_db}.obs WHERE obs_id = #{obs_id.first.our_id}")
